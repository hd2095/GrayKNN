...
def GKnn(impute_df):
    ...
    for i in range(len(empty_loc_cols)):
...
for incomplete_df in incomplete_df_list:
    ...
    #Compute NRMS
    nrmser = nrms(complete_num_df.astype('float'), imputed_num_df.astype('float'))
...
#save imputed dataframes to csv
index = 0
for df in imputed_df_list:
    df.to_csv(base_path + '/' +incomplete_df_name_list[index]+"_imputed.csv")
    index+=1
